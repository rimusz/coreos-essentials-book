...
# create an instance
gcloud compute instances create prod-control1 --project=$project --image=$image --image-project=coreos-cloud --boot-disk-size=10 --zone=$zone --machine-type=$control_machine_type --metadata-from-file user-data=cloud-config/control1.yaml --can-ip-forward --tags prod-control1 prod
