...
export PATH=${HOME}/k8s-cluster:$PATH
...
then
    ...
    mv -f etcdctl ~/k8s-cluster
    # clean up
    rm -f etcd.zip
    echo "etcdctl was copied to ~/k8s-cluster"
    echo " "
    ...
    mv -f fleetctl ~/k8s-cluster
    # clean up
    rm -f fleet.zip
    echo "fleetctl was copied to ~/k8s-cluster "
    echo " "
    ...
    mv -f kubectl ~/k8s-cluster
    #
    echo " "
    echo "kubectl was copied to ~/k8s-cluster"
    echo " "
    ...
    mv -f etcd-v$ETCD_RELEASE-linux-amd64/etcdctl ~/k8s-cluster
    # clean up
    rm -f etcd.tar.gz
    ...
    mv -f fleet-v$FLEET_RELEASE-linux-amd64/fleetctl ~/k8s-cluster
    # clean up
    rm -f fleet.tar.gz
